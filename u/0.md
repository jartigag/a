...
[^1]: [file descriptors and pipes](1.html)
[^2]: [man page sections](2.html)

## Lines

[read] `cat`\*, `grep`, `head`/`tail`, `tee`

\*: `more`, `less`

[write] `echo`/`printf`, `yes`. (`seq`, `tac`)

`awk`. `cut`, `sed`, `tr`. (`shuf`, `sort`, `uniq`, `wc`)
...
[^3]: [unix permissions](3.html)

[read] `dir`, `ls` (with colors), `pwd`/`basename`, `find`, `file`. `cksum`, {`md5,sha1,sha256`}`sum`. `strings`, `xdd` (`echo "Hello World!" | od -t x1z`)

[write] `mkdir`, `ch`{`mod,own`..} (`cp`, `mv`, `rm`, `ln`, `link`, `touch`), (`vi`, `ed`), (`mkfifo`, `mktemp`). `tar`

```sh
$ ch # Pressing Tab to autocomplete...
chage                      chmod                      chmod
chattr                     choom                      choom
chcon                      chown                      chown
chcpu                      chgpasswd                  chroot
chfn                       chgrp                      chrt
chgpasswd                  chpasswd                   chsh
chgrp                      chpasswd
chmem                      chmem
```
...
`ps`, `lsof`, `netstat`, `top`, `free`, ...

[write] `sleep`, `nice`, `kill`, `nohup`, `timeout`. `env`, `wall`. `su`, `sudo`, `who`. `dd`

## Network

`ip`, `ifconfig`, `route`
`ping`, `curl`, `ssh`. `traceroute`, `telnet`, `nmap`, `nc`, `dig`. `tcpdump`
